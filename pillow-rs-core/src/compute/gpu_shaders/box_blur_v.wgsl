// Vertical box blur pass (separable).
// Same fixed-point arithmetic as box_blur_h but summing vertically.
// For each pixel at (x,y): sum 2*radius+1 vertical neighbors (clamped),
// then: (sum * 65536 / window_size + 32768) >> 16.
//
// CPU reference (image.rs:2765-2778): vertical pass of pil_box_blur.
...
fn blur_v(x: u32, y: u32) -> u32 {
    ...
    // Sum up vertical window centered on (x,y)
    for (var dy = -radius_i32; dy <= radius_i32; dy++) {
        let sy = clamp(y_i32 + dy, 0, h_i32 - 1);
        let sample = input[sy * w + x];
