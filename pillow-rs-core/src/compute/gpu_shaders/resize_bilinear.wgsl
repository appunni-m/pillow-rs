// Bilinear resize with f32 interpolation.
// Mode-aware: for L/LA (0/1) only interpolates R channel (luma).
// CPU reference (ops/pil_resize.rs): PIL pixel-centered coordinate mapping.
//   source_center = (dx + 0.5) * src_w / dst_w
//   sx_floor = floor(source_center)
//   fx = source_center - sx_floor (fractional part)
//   result = mix(mix(p00, p10, fx), mix(p01, p11, fx), fy)
// All in f32. Round/clip to u8 at end.
//
...
fn bilinear_sample(dx: u32, dy: u32) -> u32 {
    ...
    // PIL pixel-centered source coordinate
    ...
    // For L/LA modes, only R is sampled; G/B/A are set to defaults
