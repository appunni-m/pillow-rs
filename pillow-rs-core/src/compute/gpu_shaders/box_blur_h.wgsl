// Horizontal box blur pass (separable).
// For each pixel at (x,y): sum 2*radius+1 horizontal neighbors (clamped),
// then apply i32 fixed-point division: (sum * 65536 / window_size + 32768) >> 16.
//
// CPU reference (image.rs:2737-2781): 24-bit fixed-point accumulator.
// GPU uses simplified 16-bit fixed point for WGSL i32 compliance.
//
...
fn blur_h(x: u32, y: u32) -> u32 {
    let w = params.width;
    let h = params.height;
    let radius = params.radius;
    let mode = params.mode;
    let window_size = 2u * radius + 1u;
    let idx = y * w + x;

    // Early-out: radius=0 means no blur
    if radius == 0u { return input[idx]; }

    let weight = 65536i / i32(window_size);
    let bias = 32768i;
    ...
    // Sum up horizontal window centered on (x,y)
    for (var dx = -radius_i32; dx <= radius_i32; dx++) {
        let sx = clamp(x_i32 + dx, 0, w_i32 - 1);
        let sample = input[y * w + u32(sx)];
    ...
    // Fixed-point division: (sum * weight + bias) >> 16, clamped to [0, 255]
