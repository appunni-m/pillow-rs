#!/usr/bin/env bash
# Run the exact Pillow backend fixture harness with Rust branch coverage.

set -euo pipefail

repo_dir="$(cd "$(dirname "$0")/../.." && pwd)"
coverage_dir="$repo_dir/target/coverage"
coverage_output="$coverage_dir/pillow-image-backend-rust.json"
coverage_target_dir="$coverage_dir/image-backend-target"
...
printf 'Rust image-backend coverage: %s\n' "$coverage_output"
exit "$test_status"
