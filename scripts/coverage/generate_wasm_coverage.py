...
All {WASM_EXPORT_COUNT} WASM exports use the same pillow-rs Rust code:
- **Browser**: wasm-pack --target web → ES module with fetch()
- **Server (Node.js)**: wasm-pack --target nodejs → CommonJS require()
- **Same core**: both call identical pillow-rs Rust code
- **I/O**: bytes-based (no filesystem dependency) — works everywhere

*Generated by `scripts/coverage/generate_wasm_coverage.py`*
"""
