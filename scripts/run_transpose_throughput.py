...
``--operation add`` uses the same two-image policy with default scale/offset.
...
def request(image_api: Any, core: Any, plan: dict[str, Any], data: bytes,
    ...
    try:
        ...
        elif plan.get("operation") in ("blend", "add"):
...
def receipt_error(subject: str, receipt: Any, byte_count: int, operation: str = "transpose") -> str | None:
    ...
    if backend == "gpu":
        ...
        if operation in ("blend", "add") and resource.get("auxiliary_bytes", 0) < byte_count:
...
def child(args: argparse.Namespace) -> int:
    ...
    if plan.get("operation") in ("blend", "add"):
    ...
    result: dict[str, Any] = {
        ...
        "policy": {"host_queue_depths": list(DEPTHS), "frames_per_window": FRAMES,
                   ...
                   "boundary": ("two fresh frombytes images, blend(alpha=0.3), terminal bytes, worker scheduling and receipt capture"
                                ...
                                if operation == "add" else "fresh frombytes through terminal bytes, worker scheduling and receipt capture"),
                   ...
                   "input_generator": "8192-byte tile (73*i+11*(i//17)+29)%256; "
                       ...
                       + ("; second image uses frame (j+1) modulo 16" if operation in ("blend", "add") else ""),
...
def main() -> int:
    ...
    parser.add_argument("--operation", choices=("transpose", "equalize", "invert", "blend", "add"), default="transpose")
