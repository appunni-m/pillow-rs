...
for byte addition modulo 256; ``--operation subtract-modulo`` measures byte
subtraction modulo 256 with the same policy. ``--operation logical-and``
and ``--operation logical-or`` use mode-1 inputs packed MSB-first with each row
padded to complete bytes;
fresh decoding, the logical operation and packed terminal export are timed.
...
def request(image_api: Any, core: Any, plan: dict[str, Any], data: bytes,
    ...
    try:
        ...
        if plan.get("operation") == "equalize":
        ...
        elif plan.get("operation") in ("blend", "image-blend", "add", "subtract", "multiply", "screen", "difference", "darker", "lighter", "add-modulo", "subtract-modulo", "logical-and", "logical-or", "alpha-composite"):
            ...
            image = (image_api.alpha_composite(image, other)
                     ...
                     else plan["imagechops_api"].lighter(image, other)
                     if plan["operation"] == "lighter"
                     else plan["imagechops_api"].add_modulo(image, other)
                     if plan["operation"] == "add-modulo"
                     else plan["imagechops_api"].subtract_modulo(image, other)
                     if plan["operation"] == "subtract-modulo"
                     else plan["imagechops_api"].logical_and(image, other)
                     if plan["operation"] == "logical-and"
                     else plan["imagechops_api"].logical_or(image, other)
                     if plan["operation"] == "logical-or"
...
def receipt_error(subject: str, receipt: Any, byte_count: int, operation: str = "transpose",
    ...
    if backend == "gpu":
        ...
        if operation in ("blend", "image-blend", "add", "subtract", "multiply", "screen", "difference", "darker", "lighter", "add-modulo", "subtract-modulo", "logical-and", "logical-or", "alpha-composite", "contrast", "color") and resource.get("auxiliary_bytes", 0) < byte_count:
...
def child(args: argparse.Namespace) -> int:
    ...
    if plan.get("operation") in ("blend", "image-blend", "add", "subtract", "multiply", "screen", "difference", "darker", "lighter", "add-modulo", "subtract-modulo", "logical-and", "logical-or", "alpha-composite"):
...
def run(args: argparse.Namespace) -> int:
    ...
    defaults = (["RGB", "RGBA"] if operation == "transpose" else
                ["1"] if operation in ("logical-and", "logical-or") else
                ["LA", "RGBA"] if operation == "alpha-composite" else ["L", "RGB"])
    modes = list(dict.fromkeys(args.mode or defaults))
    if operation in ("logical-and", "logical-or") and modes != ["1"]:
        raise ValueError(f"{operation} throughput requires mode 1")
    if operation not in ("logical-and", "logical-or") and "1" in modes:
        raise ValueError("mode 1 throughput currently requires logical-and or logical-or")
    ...
    result: dict[str, Any] = {
        ...
        "policy": {"host_queue_depths": list(DEPTHS), "frames_per_window": FRAMES,
                   ...
                   "boundary": ("two fresh frombytes images, blend(alpha=0.3), terminal bytes, worker scheduling and receipt capture"
                                ...
                                if operation == "add-modulo" else "two fresh frombytes images, subtract_modulo, terminal bytes, worker scheduling and receipt capture"
                                if operation == "subtract-modulo" else f"two fresh packed frombytes mode-1 images, {operation.replace('-', '_')}, packed terminal bytes, worker scheduling and receipt capture"
                                if operation in ("logical-and", "logical-or") else "two fresh frombytes images, alpha_composite, terminal bytes, worker scheduling and receipt capture"
                   ...
                   "input_generator": "8192-byte tile (73*i+11*(i//17)+29)%256; "
                       ...
                       + ("; mode 1 has ceil(width/8) MSB-first bytes per row" if operation in ("logical-and", "logical-or") else "")
                       + ("; second image uses frame (j+1) modulo 16" if operation in ("blend", "image-blend", "add", "subtract", "multiply", "screen", "difference", "darker", "lighter", "add-modulo", "subtract-modulo", "logical-and", "logical-or", "alpha-composite") else ""),
...
def main() -> int:
    ...
    parser.add_argument("--operation", choices=("transpose", "equalize", "invert", "grayscale", "convert", "putpixel", "blend", "image-blend", "add", "subtract", "multiply", "screen", "difference", "darker", "lighter", "add-modulo", "subtract-modulo", "logical-and", "logical-or", "transform", "alpha-composite", "contrast", "color", "solarize"), default="transpose")
