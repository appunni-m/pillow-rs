...
# Usage: bash scripts/bench/bench_all.sh
set -euo pipefail
...
if [ ! -f "$BENCH_DIR/pillow_baseline.json" ]; then
    ...
    python3 scripts/bench/bench_pillow_baseline.py --runs 1 --warmup 1 2>&1 | tail -2
    python3 scripts/bench/bench_baseline_add.py 2>&1 | tail -2
    echo ""
fi

# Step 3: CPU benchmarks — unified runner (reads bench_spec.json)
echo "--- CPU (Unified Runner) ---"
python3 scripts/bench/bench_unified.py --target rspil 2>&1 | tail -3
echo ""

# Step 3b: PIL baselines — same spec, Pillow side
echo "--- PIL Baseline (Unified Runner) ---"
python3 scripts/bench/bench_unified.py --target pil 2>&1 | tail -3
echo ""

# Step 3c: Cross-validate hashes
echo "--- Hash Validation ---"
python3 scripts/bench/bench_unified.py --validate 2>&1
echo ""

# Step 4: WASM benchmarks (Node.js)
echo "--- WASM CPU ---"
node scripts/bench/bench_wasm_cpu.mjs 2>&1 | tail -3
echo ""
...
echo "--- BENCHMARKS.md ---"
python3 scripts/bench/bench_aggregate.py
echo ""
