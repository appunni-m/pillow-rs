#!/usr/bin/env bash
# Run the exact shared Pillow paste corpus through the Rust API with coverage.

set -euo pipefail

repo_dir="$(cd "$(dirname "$0")/../.." && pwd)"
coverage_dir="$repo_dir/target/coverage"
coverage_output="$coverage_dir/pillow-paste-rust.json"
coverage_target_dir="$coverage_dir/paste-rust-target"
...
set +e
make -C pillow-rs test-paste-oracle
test_status=$?
set -e
...
printf 'Rust Image.paste coverage: %s\n' "$coverage_output"
exit "$test_status"
