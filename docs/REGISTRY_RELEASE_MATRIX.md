...
| pillow-rs | `pillow-rs` | `0.1.0` | crates.io | after image-slash-star and fontdone versions are visible | release branch commit `46ba6bd19`; manifest pins the pushed sibling revisions; the local release preflight builds Python/WASM artifacts and removes stale Python bytecode before wheel packaging, while the all-target Clippy gate currently reports 6,325 errors under Rust 1.96.1 and registry visibility is still required before package verification |
