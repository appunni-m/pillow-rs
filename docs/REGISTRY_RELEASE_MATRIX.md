...
| pillow-rs | `pillow-rs` | `0.1.0` | crates.io | after image-slash-star and fontdone versions are visible | manifest now pins the pushed sibling revisions; the local release preflight builds Python/WASM artifacts, while the all-target Clippy gate currently reports 6,325 errors under Rust 1.96.1 and registry visibility is still required before package verification |
