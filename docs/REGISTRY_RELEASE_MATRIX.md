...
`bf821bc67` (local tip; `origin` currently ends at `d7cb61df9`); registry
publication and the exact-commit hosted CI run remain external prerequisites.
...
`d7cb61df9`; the local checkout has two verified follow-up commits through
`bf821bc67` that are not pushed yet. `origin/main` remains at
