...
stroke implementation blocker at commit `907b27b34`
...
- run `1b9a6c76-e349-4967-890e-e028d1b6b53f`
- snapshot `beb56fe7-b020-4c44-9b04-3cec54ba6cf2`
...
`beb56fe7-b020-4c44-9b04-3cec54ba6cf2`:
...
## Current checkpoint: `getmask/getmask2(mode="1")` public parity

New commit:

- `183c35c4c` — routes `FontTextOptions.mode == "1"` through the same
  monochrome FreeType load/render flags already used by binary render helpers.
  Before this change, `getmask/getmask2` options ignored non-`RGBA` mode and
  rendered normal grayscale for Pillow inputs that require a binary mask.

Fixture updates:

- Added input-only rows:
  - `font.getmask.dejavusans20_a_getmask_1`
  - `font.getmask2.dejavusans20_a_getmask2_1`
- Expected bytes and offsets remain generated by the live Pillow oracle at
  runtime; no expected output/error payload was added to JSON.
...
- `make -C pillow-rs fmt` — passed.
- Coverage MCP command `font-tests-coverage-with-freetype`
  - run `a6603854-99b3-481d-bf18-06c0ddc8c6f3`
  - snapshot `7e9933f6-e70c-4acd-801c-334fc374dc2a`
  - commit `183c35c4c395913be5be4d147b2cf8955cfc6fc0`
  - status `passed`, coverage artifact ingested
...
| File | Lines | Branches | Functions | Regions |
|---|---:|---:|---:|---:|
| `pillow-rs/src/font/imagingft.rs` | `669/698` (`95.85%`) | `120/142` (`84.51%`) | `72/78` (`92.31%`) | `1048/1105` (`94.84%`) |
| `pillow-rs/src/font/mod.rs` | `191/191` (`100.00%`) | n/a | `41/41` (`100.00%`) | `251/253` (`99.21%`) |
...
- `974c16d5e` — added an owned Font fixture asset and input-only `getmask2`
  row for a `maxp` profile that Pillow reports as
  `OSError: too many function definitions`. This covers the corresponding
  `ft_error_to_pil` mapping branch through the live Pillow oracle.

What this closes:

- The public Font fixture corpus now covers another real FreeType/Pillow error
  lane without embedding expected output or expected error payload in JSON.
- The asset is owned under `pillow-rs/tests/fixtures/font/input/fonts`, not
  referenced from deprecated or external fixture trees.
...
- Coverage MCP command `imagingft-tests-coverage-fixed`
  - run `128abb03-894e-459d-a5d9-65c14a03533b`
  - snapshot `2eafcc4b-2b72-4d8b-bae4-e78d85dbe5c0`
  - commit `974c16d5e602f61c322847e4c2d65ea2f0060e0b`
  - status `passed`, coverage artifact ingested
...
| File | Lines | Branches | Functions | Regions |
|---|---:|---:|---:|---:|
| `pillow-rs/src/font/imagingft.rs` | `971/1036` (`93.73%`) | `194/238` (`81.51%`) | `99/110` (`90.00%`) | `1612/1740` (`92.64%`) |
...
- Runtime blocker remains `getmask/getmask2(stroke_width != 0)`. Pillow
  ...
  standards.
- Remaining `imagingft.rs` gaps include other FreeType error mappings,
...
- `226b8e748` — extended the deterministic variation-name fixture generator
  to create a second owned fixture where fvar axis/instance `name` records use
  Platform 1 records. Active input-only rows now exercise
  `get_variation_axes` and `get_variation_names` through the Platform 1
  fallback path against the live Pillow oracle.

What this closes:

- `imagingft.rs::name_bytes` now covers both fallback branches after the
  preferred Windows/en-US lookup: Windows non-en-US and Platform 1.
- The generated Platform 1 fixture is owned under
  `pillow-rs/tests/fixtures/font/input/fonts`; active Font tests still do not
  reference deprecated or external fixture assets directly.
- No expected output, hash, status, or error expectation was added to JSON.
  The rows remain pure inputs and the expected payload is generated by Pillow
  at runtime.
...
- `make -C pillow-rs font-tests` — passed
- Coverage MCP command `imagingft-tests-coverage-fixed`
  - run `6b7afe12-b4af-4660-91ff-4292eac6e241`
  - snapshot `3c85b2e5-a96c-4e4b-a68e-d76216f03ac6`
  - commit `226b8e748cad4b7422b9b4646798dec70a0710a3`
  - status `passed`, coverage artifact ingested
...
## Current checkpoint: variation-name fallback fixture coverage

New commit:

- `301b5e4c9` — added a deterministic fixture generator and owned Font fixture
  asset for variation-name fallback coverage. The generated font changes only
  the `name` table language IDs for fvar axis/instance names from Windows
  en-US (`0x0409`) to Windows non-en-US (`0x040c`), then active
  input-only rows exercise `get_variation_axes` and `get_variation_names`
  against the live Pillow oracle.

What this closes:

- `imagingft.rs::name_bytes` now covers the Windows non-en-US fallback branch
  for variation axis and instance names.
- The new fixture is owned under `pillow-rs/tests/fixtures/font/input/fonts`;
  active Font tests still do not reference deprecated or external fixture
  assets directly.
- No expected output, hash, status, or error expectation was added to JSON.
  The rows remain pure inputs and the expected payload is generated by Pillow
  at runtime.
...
- `make -C pillow-rs font-tests` — passed
- Coverage MCP command `imagingft-tests-coverage-fixed`
  - run `b5e609d3-3bb4-4bf5-87fe-2897c15aad3e`
  - snapshot `9c036b4c-3946-494f-a903-fef24c462e0a`
  - commit `301b5e4c9a0bb810685ccd67d5645b8db1afdae9`
  - status `passed`, coverage artifact ingested
...
| File | Lines | Branches | Functions | Regions |
|---|---:|---:|---:|---:|
| `pillow-rs/src/font/imagingft.rs` | `962/1036` (`92.86%`) | `188/238` (`78.99%`) | `97/110` (`88.18%`) | `1603/1740` (`92.13%`) |
| `pillow-rs/src/font/mod.rs` | `258/282` (`91.49%`) | n/a | `57/64` (`89.06%`) | `331/373` (`88.74%`) |
...
- Remaining `imagingft.rs` gaps include FreeType error mappings,
  request-size error/fallback branches, render clipping/bitmap guard branches,
  Platform 1 name fallback selection, and stroked mask rendering.
- Remaining `font/mod.rs` reported gaps are source-map lines on public option
  declarations/doc comments, not uncovered public method bodies.
