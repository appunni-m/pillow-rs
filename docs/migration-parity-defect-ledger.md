...
| Live parity | The current committed run executes all 1,214 indexed cases: 1,214 pass / 0 fail / 0 infrastructure errors / 0 not-run. Coverage-gap review found and fixed real divergences: `ImageChops.invert` for LA/RGBA, and `ImageColor` alpha handling (explicit alpha dropped or forced to 255, mode-`1` thresholded instead of ITU-R 601 graylevel) | RUN-001, RUN-004, RUN-006, RUN-007, and RUN-009 have maintained producers/boundaries. The result is live source-vs-target evidence, not a fixture replay. RUN-008 remains a classification/anti-cheat follow-up for separate regression tests that intentionally retain oracle fixtures. |
| Coverage | `make migration-parity-coverage-rust` is the maintained merged Python+Rust lane: it temporarily instruments the extension, executes all 22 indexed plans (1,214 passed / 0 failed), and emits a strict `coverage-result@1` with per-file function/line/branch/region dimensions. Declared-path measurement: 1,126/2,180 functions (51.7%), 14,774/25,327 lines (58.3%), 1,865/4,300 branches (43.4%), 19,478/35,121 regions (55.5%); therefore code coverage is not 100%. The region-coverage report (`make migration-parity-region-coverage`) lists the 165/204 operations below 90% ascending; worst is image-ops at 41.5%, image-font 46.1%, image-sequence 55.9%, image-draw 56.4%, image-core 58.1% (`PIL.Image.Image.getbbox -> 4,034/6,940`), image-filter 61.5%, image-color/palette 66.1%. No managed snapshot has been ingested, so aggregate coverage outcomes remain `not_proven` | COV-001 and COV-002 now have strict producers and regression checks; nuanced cases are selected by coverage plans and drive the measured dimensions (color.rs regions 192/805 -> 614/929 after the convert/color case batch). Managed proof still requires explicit Coverage MCP approval and a fresh snapshot of the new artifact. Closing the remaining measured gap to the declared 100% thresholds is the long-pole code work, bucket by bucket from the ascending report. |
