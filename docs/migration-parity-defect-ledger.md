...
| Coverage | `make migration-parity-coverage-rust` is the maintained merged Python+Rust lane: it temporarily instruments the extension, executes the 22 indexed plans plus the coverage-only font-native command (1,701 tests: 1,244 parity workflows + 457 native font cases; 0 failures), and emits a strict `coverage-result@1` with per-file function/line/branch/region dimensions. Declared-path measurement: 1,417/2,180 functions (65.0%), 17,393/25,371 lines (68.6%), 2,371/4,324 branches (54.8%), 22,816/35,133 regions (64.9%); therefore code coverage is not 100%. The font-native command is the maintained port of the deprecated `font_public_api_v0` corpus (the reference that previously drove imagingft.rs to 100%): it raised imagingft.rs regions 1,002/1,444 (69.4%) -> 1,209/1,444 (83.7%) and pilfont.rs 146/554 (26.4%) -> 509/554 (91.9%). The remaining imagingft.rs regions are legacy core-API variants the PyO3 binding bypasses (`getlength`/`getmask2`/`getmask2_with_start`, `native_getvaraxes`/`native_getvarnames`/`native_setvaraxes`/`native_setvarname`, stroker bitmap paths, `raw_name_bytes` fallback, wrapper-pre-empted layout validation) — reachable only by a Rust exercise test, which is the next step. The region-coverage report (`make migration-parity-region-coverage`) lists the 165/204 operations below 90% ascending; worst is image-ops at 46.3%, image-core 59.6% (`PIL.Image.Image.getbbox -> 4,134/6,940`), image-filter 61.5%, image-color/palette 66.1%. Known unreachable surface: `Image.getexif` is a core stub (empty bytes), so `exif_get_orientation`/`exif_remove_orientation` and EXIF-carrying JPEG parity are unverified until EXIF extraction is implemented. No managed snapshot has been ingested, so aggregate coverage outcomes remain `not_proven` | COV-001 and COV-002 now have strict producers and regression checks; nuanced cases are selected by coverage plans, and the maintained font-native command covers the internal paths the PIL surface cannot reach. Managed proof still requires explicit Coverage MCP approval and a fresh snapshot of the new artifact. Closing the remaining measured gap to the declared 100% thresholds is the long-pole code work, bucket by bucket from the ascending report. |
