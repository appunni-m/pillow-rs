...
| Live parity | The current committed run executes all 1,190 indexed cases: 1,190 pass / 0 fail / 0 infrastructure errors / 0 not-run. A coverage-gap review found and fixed a real divergence: `ImageChops.invert` raised OSError for LA/RGBA while Pillow 12.2.0 inverts both modes; new nuanced cases lock the fixed behavior | RUN-001, RUN-004, RUN-006, RUN-007, and RUN-009 have maintained producers/boundaries. The result is live source-vs-target evidence, not a fixture replay. RUN-008 remains a classification/anti-cheat follow-up for separate regression tests that intentionally retain oracle fixtures. |
| Coverage | `make migration-parity-coverage-rust` is the maintained merged Python+Rust lane: it temporarily instruments the extension, executes all 22 indexed plans (1,190 passed / 0 failed), and emits a strict `coverage-result@1` with per-file function/line/branch/region dimensions. Declared-path measurement: 1,058/2,165 functions (48.9%), 14,125/25,153 lines (56.2%), 1,789/4,234 branches (42.2%), 18,096/34,749 regions (52.1%); therefore code coverage is not 100%. `image-stat` lines/branches are 86/86 and 12/12; `image-chops` lines are 156/156 with one unexecutable let-pattern branch arm remaining. No managed snapshot has been ingested, so aggregate coverage outcomes remain `not_proven` | COV-001 and COV-002 now have strict producers and regression checks; nuanced cases are selected by coverage plans and drive the measured dimensions. Managed proof still requires explicit Coverage MCP approval and a fresh snapshot of the new artifact. Closing the remaining measured gap to the declared 100% thresholds is the long-pole code work. |
