...
| Coverage | `make migration-parity-coverage-rust` is the maintained merged Python+Rust lane: it temporarily instruments the extension, executes the 22 indexed plans plus the coverage-only font-native command and the Rust exercise test (1,706 tests: 1,249 parity workflows + 457 native font cases; 0 failures), and emits a strict `coverage-result@1` with per-file function/line/branch/region dimensions. Declared-path measurement: 1,521/2,180 functions (69.8%), 18,076/25,377 lines (71.2%), 2,449/4,332 branches (56.5%), 23,825/35,113 regions (67.9%); therefore code coverage is not 100%. The font-native command + Rust exercise test are the maintained port of the deprecated `font_public_api_v0` corpus: imagingft.rs regions 1,002/1,444 (69.4%) -> 1,393/1,444 (96.5%) and pilfont.rs 146/554 (26.4%) -> 509/554 (91.9%); only two imagingft.rs defensive branches remain unreachable (unknown-FT-error fallback; FT_Set_Var_Design_Coordinates error). The filter bucket is closed: filter.rs regions 59/96 (61.5%) -> 72/76 (94.7%), with four defensive core lines superseded by wrapper validation. The region-coverage report (`make migration-parity-region-coverage`) lists the 165/204 operations below 90% ascending; worst is image-ops at 46.3%, image-draw 56.4%, image-sequence 57.2%, image-core 59.6% (`PIL.Image.Image.getbbox -> 4,134/6,940`), image-color/palette 66.1%. Known unreachable surface: `Image.getexif` is a core stub (empty bytes), so `exif_get_orientation`/`exif_remove_orientation` and EXIF-carrying JPEG parity are unverified until EXIF extraction is implemented. No managed snapshot has been ingested, so aggregate coverage outcomes remain `not_proven` | COV-001 and COV-002 now have strict producers and regression checks; nuanced cases are selected by coverage plans, the font-native command covers internal paths the PIL surface cannot reach, and the Rust exercise test measures the legacy core variants. Managed proof still requires explicit Coverage MCP approval and a fresh snapshot of the new artifact. Closing the remaining measured gap to the declared 100% thresholds is the long-pole code work, bucket by bucket from the ascending report. |
