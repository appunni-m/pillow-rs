...
| Coverage | `make migration-parity-coverage-rust` is the maintained merged Python+Rust lane: it temporarily instruments the extension, executes all 22 indexed plans (1,169 passed / 0 failed), and emits a strict `coverage-result@1` with per-file function/line/branch/region dimensions. Declared-path measurement: 1,058/2,165 functions (48.9%), 14,061/25,156 lines (55.9%), 1,773/4,238 branches (41.8%), 17,998/34,757 regions (51.8%); therefore code coverage is not 100%. No managed snapshot has been ingested, so aggregate coverage outcomes remain `not_proven` | COV-001 and COV-002 now have strict producers and regression checks, and the previous ad-hoc LLVM run is replaced by the maintained lane. Managed proof still requires explicit Coverage MCP approval and a fresh snapshot of the new artifact. Closing the measured gap to the declared 100% thresholds is the remaining code work. |
