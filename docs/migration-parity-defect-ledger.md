...
| COV-010 | medium | Derived images retain the opened container's `format` (e.g. `format == "PNG"` on `getchannel`/`resize`/`convert` results), while Pillow sets `format` to `None` on every derived image. Pixel bytes match; the parity image comparator fails on the `format` field, blocking asset-based cases for image-valued operations (getchannel, resize, crop, rotate, convert, ...). Found by opened-asset getchannel/resize cases. | Do not propagate `source_format` to derived pipeline results (Pillow keeps format only on the originally opened image), and re-enable image-valued asset cases. |
