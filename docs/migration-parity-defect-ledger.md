...
| COV-009 | medium | `Image.convert` dropped palette-transparency metadata: Pillow carries `info["transparency"]` through P-to-RGB/L by converting the transparency index through the palette (index 1 with palette (255,0,0) yields `{"transparency": (255, 0, 0)}` for RGB and the gray level for L), and drops it for alpha modes. The Rust convert result carried no info. RESOLVED: `Image::converted_palette_transparency` computes the transformed value (palette color for RGB, BT.601 gray for L, None for alpha modes and byte tables), exposed through a PyO3 binding, and the wrapper convert sets `info["transparency"]` on the result. P-to-LA/PA conversion itself remains unsupported by the target (separate convert-mode gap). | Keep the re-enabled convert-on-opened-transparency parity cases green; track P-to-LA/PA support separately. |
