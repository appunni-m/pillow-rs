...
| COV-009 | medium | `Image.convert` dropped palette-transparency metadata: Pillow carries `info["transparency"]` through P-to-RGB/L by converting the transparency index through the palette (index 1 with palette (255,0,0) yields `{"transparency": (255, 0, 0)}` for RGB and the gray level for L), and drops it for alpha modes. RESOLVED: `Image::converted_transparency_info` and `Image::converted_compatibility_info` now compute the scalar, byte, or tuple value in the Rust core; PyO3 only marshals the host value and the Python wrapper merges the returned mapping. The follow-on P-to-LA/PA gaps were also fixed: P-to-LA now carries palette transparency into the alpha band and P-to-PA keeps palette indices with the palette alpha, and the convert path now uses the decoded mode for lazy Bytes images so palette-aware conversion runs before load(). Convert cases for P-to-LA/PA/RGB/RGBA/L on opened assets all pass. | Keep the convert-on-opened-transparency parity cases green. |
...
| COV-011 | medium | A valid little-endian 16-bit TIFF decodes through `image-slash-star` as Pillow mode `I;16`. Pillow's `Image.getcolors()` then raises `ValueError("image has wrong mode")`, while the current downstream Rust raster can materialize and return a result. The mismatch is at the upstream 16-bit mode/codec boundary and is not fixed by adding a local `getcolors` branch. | PENDING UPSTREAM: keep the TIFF stimulus and first-divergence evidence out of the active parity lane until `image-slash-star` lands its mode behavior fix; then re-enable an input-only error-parity case and verify the downstream mode contract. |
| COV-012 | medium | A valid indexed PNG may contain a `tRNS` transparency table without a `PLTE` chunk. Pillow preserves the table on open, and `Image.apply_transparency()` then raises `IndexError("list assignment index out of range")` because the palette is empty. The current `image-slash-star` decoder drops the `tRNS` metadata at decode time, so the downstream call returns without reaching the Rust transparency path. | PENDING UPSTREAM: keep this stimulus out of the active parity lane until `image-slash-star` preserves `tRNS` for indexed images without `PLTE`; then re-enable the case and verify the Rust error path. |
