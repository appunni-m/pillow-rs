...
| Coverage | All 22 indexed plans execute locally: 1,169 passed / 0 failed. The target coverage report is produced, but no managed snapshot has been ingested, so aggregate coverage outcomes remain `not_proven` | COV-001 and COV-002 now have strict producers and regression checks. Managed proof still requires explicit Coverage MCP approval and a fresh snapshot. Rust LLVM diagnostics are only 1,063/2,024 functions, 11,019/23,415 lines, and 17,839/40,403 regions (branch dimension 0/0); therefore code coverage is not 100%. Local Python dimensions are diagnostic until ingestion. |
