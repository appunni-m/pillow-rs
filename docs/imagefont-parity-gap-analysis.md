...
- The current BGRA SBIT fixture includes an alpha-zero pixel generated by `pillow-rs-freetype/scripts/build_sbit_fixtures.py`. Existing live-oracle `getmask`/`getmask2` rows now prove Pillow-compatible transparent color bitmap conversion, moving `imagingft.rs` from 1686 to 1687 covered lines and from 252 to 253 covered branches in the pre-libraqm-helper snapshot; after centralizing the unsupported-libraqm constructor, the current line metric is 1684/1709 with the same 253/262 branch coverage and 2619/2718 region coverage.
...
| `pillow-rs/src/font/imagingft.rs` | 1684/1709 98.54% | 253/262 96.56% | 165/176 93.75% | 2619/2718 96.36% | real partial branch gaps remain |
