...
- The lower pending-route diagnostic is now maintained as `make -C pillow-rs-freetype test-pending-case CASE=<exact-case-id>` instead of requiring temporary edits to `scripts/check_public_api_inputs.py`. Normal parity classification is unchanged unless `FONTDONE_UNIFIED_INCLUDE_PENDING_CASE` exactly matches one pending `case_id`. Running `make -C pillow-rs-freetype test-pending-case CASE=ftglyph.FT_Glyph_To_Bitmap.pending_stroked_mono_target_outline_to_bitmap` promotes only that blocker row and fails honestly on `rust ffi:status`: pinned C returns `FT_Err_Ok`, current Rust returns `FT_Err_Unimplemented_Feature (7)` because the unverified closed/curve path guard still protects the non-exact lower stroker output.
