...
Rust commit reviewed: `683bca1d`

Coverage MCP run: `3b3e6b27-19f2-4d0c-a04f-954dc254df6a`

Coverage MCP snapshot: `8ed53fb9-6288-47c6-9bfc-8072909441c8`
...
- 363 input-only rows execute.
- 363 rows match live Pillow 12.2.0 exactly.
...
- Coverage MCP command `imagingft-tests-coverage-fixed` passes and ingests snapshot `8ed53fb9-6288-47c6-9bfc-8072909441c8`.
...
Coverage snapshot: `8ed53fb9-6288-47c6-9bfc-8072909441c8`
from Coverage MCP run `3b3e6b27-19f2-4d0c-a04f-954dc254df6a`
at commit `683bca1d545b4251ddb580a366bf4520cdb37118`.
...
- Lines: 3937/27302, 14.42%
- Branches: 365/4682, 7.80%
- Functions: 373/1986, 18.78%
- Regions: 5891/47275, 12.46%
...
| `375` | Optional family/style names in `getname_optional`. | `FreeTypeFont.getname()` returns C font family/style values. | Current active fonts provide names. A missing-family/style font would be a legitimate public row if Pillow loads it and returns `None`/empty consistently. | Candidate fixture gap: find or generate a maintained font with absent family/style names that Pillow 12.2.0 accepts. |
| `453`, `511`, `519`, `537`, `541` | Variation `font_variant`, `set_variation_by_name`, and `set_variation_by_axes` option/error propagation. | `ImageFont.py` forwards to `_imagingft` variation APIs. | Existing rows cover normal variation names/axes and some public errors. Remaining status paths require lower FreeType to return errors from `FT_Set_Named_Instance` or `FT_Set_Var_Design_Coordinates` after preliminary validation passes. | Add rows only with real variable-font assets that trigger those FreeType statuses under Pillow. If the lower implementation lacks the exact status, add it in `pillow-rs-freetype` minimally. |
