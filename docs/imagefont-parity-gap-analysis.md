...
Post Stroker_Set attribute route Coverage MCP run: `ec803b30-9fde-4a68-8ad0-e192ab6e60a0`
...
Post remaining static-error route sweep: Pillow 12.2.0 over 63 candidate
...
Post missing variation style Coverage MCP run: `df29cc88-7d5e-43aa-819d-e3900fd743a5`

Post missing variation style Coverage MCP snapshot: `5a3fe641-c100-4557-a58e-e42d746aeae4`

Post setvarname invariant cleanup Coverage MCP run: `8d1b8220-c9ee-49ed-a707-42cc6add4b68`

Post setvarname invariant cleanup Coverage MCP snapshot: `a9f5420f-a3cc-4977-a14b-e4e99c1ea527`

Post anchor error helper Coverage MCP run: `fe35229b-781f-4fe3-8913-590c5a6839a8`

Post anchor error helper Coverage MCP snapshot: `da0600ad-8a8d-4387-bce0-c5bbfd36d163`

Post binary render cleanup audit Coverage MCP run: `2fda1cb9-c908-4ee7-a8d4-eb822602e5d3`
...
Post getmask stroke_filled scope audit Coverage MCP run: `5a180e08-7ee5-40d3-8a4b-b9e90b4ab7f2`
...
- Commit `c9c141856` adds four input-only native variation fallback rows for
  `_imagingft.Font.getvarnames/getvaraxes` using the maintained
  `variable-name-windows-fallback.ttf` and
  `variable-name-platform1-fallback.ttf` fixtures. These rows prove the raw
  native name-record fallback path separately from the higher-level
  `FreeTypeFont.get_variation_*` decoded/filtered API and increase the active
  corpus from 441 to 445 rows. A probe of
  `variable-name-missing-subfamily.ttf` showed Pillow 12.2.0 native
  `font.font.getvarnames()` can return NULL list entries and segfault when the
...
- Commit `9896080d7` records and reverts a measured source-shape cleanup for
  ...
  new uncovered source line in `length_from_basic_layout`. The compact form is
  therefore kept until a fixture-backed Pillow-visible behavior gap justifies a
  real implementation change.
...
- Additional mono BASIC rows for `AV` and `jQ` now prove live-oracle parity for normal-vs-mono load-flag behavior across `getlength`, `getbbox`, `getmask`, and `getmask2`. Coverage MCP shows these rows are semantically useful but do not reduce the remaining LLVM-reported `imagingft.rs` region gaps.
- Commit `fd0bb7ccafd8968031e962c1f3e12c5102a5e5f0` moved `FT_Stroker_ParseOutline` from a two-point-line-only parser to the FreeType 2.14.3 contour/tag control flow that delegates line, conic, and cubic segments to the existing segment routes. Later focused checks show the public ImageFont stroke dependencies now pass for the active corpus (`FT_Glyph_Stroke` 8/8, `FT_Glyph_StrokeBorder` 4/4, and `FT_Glyph_To_Bitmap` 11/11). Remaining lower stroker pending rows are lower FreeType parity work unless a new live Pillow Font row proves public visibility.
...
- Commit `384a4139a07aa8b5f09486a1f034ba5fbcb9541b` originally added a public `getlength` row using the maintained lower `hinter-execution-too-long-loop.ttf` fixture. Current Pillow 12.2.0 probes at the row's public size (`7`) show `getlength`, `getbbox`, `getmask`, and `getmask2` all succeed, so the active case is now named `font.getlength.hinter_execution_loop_success`. It is valid public parity coverage for the font asset, but it is not proof that `PIL.ImageFont` emits `FT_Err_Execution_Too_Long`; the static table-entry line remains LLVM-uncovered.
...
- Python ABI oracle coverage now includes
  `tests/test_imagefont_facade_oracle.py`: twenty-one strict-cover tests compare
  `pillow-rs-py` against Pillow 12.2.0 for module load functions, `Layout`,
  `MAX_STRING_LENGTH`, `truetype` path-like and byte-path inputs, file-like
  variation names/axes, both variation setters, file-like `font_variant(size=...)`,
...
- Follow-up native-object facade coverage: `FreeTypeFont.font` now points to
...
- Native `setvarname` missing-name parity fix: Pillow 12.2.0 accepts a named
  instance whose subfamily record normalizes to empty and then exposes
  `FreeTypeFont.getname()` style as `None`, not `"Regular"` or `""`. Active
  row `font.native_setvarname.missing_subfamily_name_keeps_none` uses the
  maintained `variable-name-missing-subfamily.ttf` fixture to pin that behavior
  through the live oracle. Root `imagefont_getname` and the Python facade now
  preserve optional family/style values; JS returns `null` array entries for
  missing names.
...
- The focused maintained command is now `make test-imagefont-facade`. It runs
...
- Follow-up `TransposedFont` facade coverage: `make test-imagefont-facade` now
  also covers `ImageFont.TransposedFont.getbbox`, `getlength`, and `getmask`
  against Pillow 12.2.0 for both FreeType-backed and bitmap fonts across all
  supported transpose orientations. The check compares successful outputs and
  Pillow-shaped errors directly; no expected output is stored in the test data.
- Follow-up stroked-mask parity fix: adding `jQ` with `stroke_width=1.5` to
  `FreeTypeFont.getmask`/`getmask2` facade checks exposed a real allocation
  mismatch. Pillow allocates stroked masks from
  `_imagingft.c::bounding_box_and_anchors`; Rust now uses the bbox-derived
  stroked left edge for the canvas origin and the active input-only corpus has
...
| `ImageFont.TransposedFont` | `getmask`, `getbbox`, `getlength` | Rust core exposes helper operations (`get_transposed_mask`, `transposed_bbox`, `validate_transposed_length`); the Python facade now models Pillow's wrapper class and is covered by focused Pillow 12.2.0 oracle tests for FreeType and bitmap fonts. |
...
- Lines: 18112/53091, 34.12%
- Branches: 3061/11190, 27.35%
- Functions: 1389/3716, 37.38%
- Regions: 25871/81730, 31.65%
...
|---:|---|---|---|---|
| `49`, `51`, `74`, `75`, `77`, `80`, `84`, `88` | Constructor argument/error propagation and `FT_Request_Size` setup/return path. | `_imagingft.c::getfont` validates size, creates the face, then calls `FT_Request_Size`. | The fractional negative-size row covers the non-integral size message path. A current sweep across all maintained `.ttf`/`.otf` Font assets at sizes `0.0001`, `0.001`, `0.01`, `0.1`, `0.25`, `0.5`, `0.75`, `1`, `1.25`, `1.5`, `2`, and `3` found no new public route where `FT_New_Memory_Face` succeeds and `FT_Request_Size` fails with a distinct status. The observed Pillow 12.2.0 behaviors remain the known categories: tiny TrueType faces fail with `OSError("invalid ppem value")`, the active CFF row accepts tiny sizes, malformed lower fonts fail later in public text methods, and normal sub-pixel sizes produce ordinary bbox/length deltas. The remaining constructor markers are therefore mostly `?`/data-construction instrumentation and the rare unobserved `FT_Request_Size` error path. | Add a fixture only if a real Pillow 12.2.0 font/size combination naturally makes `FT_Request_Size` fail after `FT_New_Memory_Face` succeeds with a new public status. Do not fake this in the runner or add duplicate sub-pixel success rows. |
...
| `453`, `511`, `519`, `537`, `541` | Variation `font_variant`, `set_variation_by_name`, and `set_variation_by_axes` option/error propagation. | `ImageFont.py` forwards to `_imagingft` variation APIs. | Existing rows cover normal variation names/axes and public errors, including malformed fvar axis-size setter parity. Coverage line data for snapshot `eb41f17c-7e76-453d-90ba-ab996a041dbf` shows `set_variation_by_axes` pre-validation and normal/error outcomes are hit, but lines `537` and `541` remain uncovered because malformed lower fixtures return before the adapter observes a nonzero `FT_Set_Var_Design_Coordinates` status. A current Pillow 12.2.0 subprocess sweep across 41 maintained variable-font candidates and 779 public variation operations found no additional safe setter-status route: the public outcomes were success, `ValueError(b'...' is not in list)`, or `OSError("invalid argument")` from malformed fvar/name data before a distinct setter status became observable. The same sweep also confirmed several malformed/native variation cases can segfault Pillow itself, so those rows must stay out of the active corpus unless the harness grows an isolated crash-status contract. | Add rows only with real variable-font assets that trigger nonzero statuses from `FT_Set_Named_Instance` or `FT_Set_Var_Design_Coordinates` after preliminary validation passes. If the lower implementation lacks the exact status, add it in `pillow-rs-freetype` minimally. |
...
|---:|---|---|---|
| `91` | partial branch on `Ok(FreeTypeFont { engine })` | Constructor success is heavily hit; marker is return/source-map instrumentation after `FT_Request_Size`/face setup. A live Pillow 12.2.0 constructor sweep records the real distinction that tiny TrueType sizes (`0.0001` through `0.1`) fail with `OSError("invalid ppem value")`, while the CFF fixture accepts the same tiny sizes and returns a normal public bbox. Active row `font.getbbox.pure_cff_tiny_size_success` covers that success path without stored expected output. Current snapshot `da0600ad-8a8d-4387-bce0-c5bbfd36d163` still reports this as a one-branch source-map marker. | Add only a real font/size row if Pillow reaches another distinct constructor branch. |
| `253` | uncovered static tuple-start line | `FT_Err_Execution_Too_Long` table entry. Current Pillow 12.2.0 probes with the maintained `hinter-execution-too-long-loop.ttf` asset at public size `7` return normal `getlength`/`getbbox`/`getmask`/`getmask2` results, so the active Font row is a success-path parity row, not an execution-too-long error row. A broader 63-font Pillow sweep across `post`, `execution`, `loop`, `hinter`, `fpgm`, `prep`, `idef`, `instr`, and related lower assets tried constructor, `getname`, `getmetrics`, `getlength`, `getbbox`, `getmask`, and `getmask2` at sizes `0.0001`, `0.001`, `0.01`, `0.1`, `1`, `2`, `7`, `12`, `20`, and `64`; the observed public errors were only `invalid ppem value`, `code overflow`, `too many instruction definitions`, and `nested DEFS`. | Do not add rows for this table entry unless a pinned FreeType/Pillow public route emits the exact error. |
| `271` | uncovered static tuple-start line | `FT_Err_Post_Table_Missing` table entry. Lower FreeType oracle rows prove absent optional `post` table is surfaced by public glyph-name APIs as `FT_Err_Invalid_Argument`, not `FT_Err_Post_Table_Missing`; no current Pillow `ImageFont` path is known to emit this code. Direct Pillow 12.2.0 probes with `post-missing.ttf`, `no-post-names.ttf`, `post-format-unsupported.ttf`, and `invalid-post-format.ttf` all load and return normal public `getname`/`getbbox`/`getlength`/`getmask` results. The same 63-font sweep above did not produce `OSError("PostScript (post) table missing")` through any tested public ImageFont route. | Do not add ImageFont rows for absent `post` tables. Revisit only if a pinned FreeType/Pillow public route emits this exact code. |
| `796` | partial branch on `render_text_binary` packing call | Active `render_text_binary` rows cover normal text, empty text, zero-height space, SBIT, and error paths. Snapshot `da0600ad-8a8d-4387-bce0-c5bbfd36d163` records line `796` hit 24 times with `3/4` branch counters; adjacent call/return lines are hit. This is `?`/call-boundary source mapping around `mask_from_run_with_start(...)` and `pack_rgba(...)`, not a distinct Pillow-visible binary-render behavior. | No duplicate binary-render rows unless a new Pillow case changes the RGBA packing payload or error class. |
| `826`, `829` | partial markers inside anchor dispatch | Active rows cover missing anchor, bad-length anchor, valid horizontal/vertical anchors, invalid horizontal anchor, and invalid vertical anchor. Centralizing the repeated error construction removed the false uncovered line markers; current snapshot shows the bad-anchor helper body is covered. Remaining labels are source-map accounting on the `let Some(anchor)`/length-check span. | No duplicate anchor rows. Revisit only if a new Pillow anchor string reaches a distinct message or offset behavior. |
| `928` | partial marker on `length_from_basic_layout` helper boundary | Active `getlength`, native `getlength`, bbox, mask, and render rows route through BASIC layout length. Current snapshot records this helper hit 1225 times; the marker is helper-boundary source mapping, not a distinct Pillow behavior gap. | No duplicate length rows unless reverse comparison finds a new BASIC-layout behavior difference. |
...
The current implementation is good enough to trust the active 445-row Font fixture corpus.
...
- Latest measured code Coverage MCP audit after `defcf0a5a`: managed command
  ...
  did not regress the core Font/ImageFont parity corpus or create a new
...
- Follow-up audit after `022ead7d1`: Coverage MCP snapshot
  ...
  missing Pillow `_imagingft.c` adapter behavior. A follow-up Pillow 12.2.0
  probe showed the existing `hinter-execution-too-long-loop.ttf` Font row is a
  normal success path, not a public `FT_Err_Execution_Too_Long` error path, so
  adding more static error-table rows is not justified unless a real public
  route emits that exact status.
...
- `FT_Stroker_ParseOutline.line_conic_cubic_success` route promotion: the row
  ...
  branches, `162/173` functions, and `2612/2696` regions with the same seven
  markers: `91`, `253`, `271`, `796`, `826`, `829`, and `928`.
- `FT_Stroker_Set.attributes_affect_geometry` route promotion: the row now
  uses the input-only
  `pillow-rs-freetype/tests/fixtures/input/outlines/stroker/set-attribute-matrix.json`
  asset and has a maintained same-input runtime route through pinned C
  FreeType, Rust FFI, C ABI, and WASM ABI. The route keeps the matrix bounded
  while covering all declared families at least once: radius values `32`,
  `128`, and `384`; cap families `BUTT`, `ROUND`, and `SQUARE`; join families
  `ROUND`, `BEVEL`, `MITER_VARIABLE`, and `MITER_FIXED`; and path shapes
  `open_line`, `acute_corner`, `conic_curve`, and `cubic_curve`. It compares
  exact `status_sequence`, `point_count`, `contour_count`, exported
  `points/tags/contours`, and `cbox`, with no expected output stored in the
  input JSON. Verification:
  `make -C pillow-rs-freetype test-pending-case
  CASE=ftstroke.FT_Stroker_Set.attributes_affect_geometry` passes `1/1`, and
  normal `make -C pillow-rs-freetype test-case CASE=ftstroke.FT_Stroker_Set`
  passes `4/4` with `0` pending. Route audit moves to `real-parity=4862`,
  `pending-route=160`. This is lower FreeType stroker attribute/geometry
  route parity and does not claim a new direct `imagingft.rs` adapter region.
  Coverage MCP command `font-tests-coverage-with-freetype-pillow-12-2` passed
  after this route as run `ec803b30-9fde-4a68-8ad0-e192ab6e60a0` and ingested
  snapshot `062c2ec9-22f6-4a83-830f-d69db8fe87d3` for commit `62d94bbdd`.
  Direct `pillow-rs/src/font/imagingft.rs` remains `1666/1688` lines,
  `249/254` branches, `162/173` functions, and `2612/2696` regions with the
  same seven markers: `91`, `253`, `271`, `796`, `826`, `829`, and `928`.
