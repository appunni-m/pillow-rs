// Difference: abs(a - b) per channel
// Uses conditional subtraction for unsigned integer abs
...
fn main(@builtin(global_invocation_id) gid: vec3<u32>) {
    ...
    let out_r = uabs_diff(ar, br);
    let out_g_raw = uabs_diff(ag, bg);
    let out_b_raw = uabs_diff(ab, bb);
    let out_a_raw = uabs_diff(aa, ba);
