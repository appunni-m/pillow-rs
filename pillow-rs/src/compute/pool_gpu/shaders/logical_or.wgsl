// LogicalOr: either stored sample nonzero => 255, otherwise zero.
...
fn main(@builtin(global_invocation_id) gid: vec3<u32>) {
    ...
    if params.mode == 9u {
        ...
        // Truth commutes with OR, so combine stored samples before one
        // canonicalization. Low-seven-bit sums cannot carry between bytes.
        let word = input_a[idx] | input_b[idx];
    ...
    let out_r = select(0u, 255u, (ar | br) != 0u);
    let out_g_raw = select(0u, 255u, (ag | bg) != 0u);
    let out_b_raw = select(0u, 255u, (ab | bb) != 0u);
    let out_a_raw = select(0u, 255u, (aa | ba) != 0u);
