// LogicalAnd: both stored samples nonzero => 255, otherwise zero.
...
fn truth_bytes(word: u32) -> u32 {
    // Each low-seven-bit sum is at most 254, so no carry crosses byte lanes.
    let flags = (((word & 0x7f7f7f7fu) + 0x7f7f7f7fu) | word) & 0x80808080u;
    return (flags >> 7u) * 255u;
}
...
fn main(@builtin(global_invocation_id) gid: vec3<u32>) {
    ...
    if params.mode == 9u {
        ...
        // Public writes can retain values other than 0/255 in mode 1. Test
        // every byte for truth before AND and return canonical result samples.
        output[idx] = truth_bytes(input_a[idx]) & truth_bytes(input_b[idx]);
        return;
    }
