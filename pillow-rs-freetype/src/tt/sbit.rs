...
impl SbitTable {
    pub fn load_glyph_status(
        ...
        strike.find_image(&self.eblc, glyph_index, recurse_count)
    }
}
...
impl SbitStrike {
    fn find_image(
        ...
        for range_index in 0..count {
            ...
            return find_image_in_subtable(eblc, subtable_start, start, glyph_index, recurse_count);
        }
...
fn find_image_in_subtable(
    ...
    match index_format {
        1 => {
            ...
            image_found_or_missing(image_start, image_end, recurse_count)
        }
        3 => {
            let offset_index = usize::from(glyph_index - first_glyph);
...
fn image_found_or_missing(
    ...
    if image_start == image_end || image_start > image_end {
        return Err(no_bitmap_error(recurse_count));
    }
    Ok(())
}
