...
use super::blue_strings::{BlueStringEntry, SCRIPT_TABLE, SCRIPT_LATN, SCRIPT_GREK};
...
pub static GRK_RANGES: &[UniRange] = &[
    UniRange { first: 0x0370, last: 0x03FF },  // Greek and Coptic
    UniRange { first: 0x1F00, last: 0x1FFF },  // Greek Extended
];

/// Unicode ranges for Latin script (matching afranges.c: latn_uniranges).
pub static LATN_RANGES: &[UniRange] = &[
    UniRange { first: 0x0020, last: 0x007F },  // Basic Latin
    UniRange { first: 0x00A0, last: 0x00A9 },  // Latin-1 Supplement
    UniRange { first: 0x00AB, last: 0x00B1 },
    UniRange { first: 0x00B4, last: 0x00B8 },
    UniRange { first: 0x00BB, last: 0x00FF },
    UniRange { first: 0x0100, last: 0x017F },  // Latin Extended-A
    UniRange { first: 0x0180, last: 0x024F },  // Latin Extended-B
    UniRange { first: 0x0250, last: 0x02AF },  // IPA Extensions
    UniRange { first: 0x02B9, last: 0x02DF },  // Spacing Modifier Letters
    UniRange { first: 0x02E5, last: 0x02FF },
    UniRange { first: 0x0300, last: 0x036F },  // Combining Diacritical
    UniRange { first: 0x1AB0, last: 0x1ABE },
    UniRange { first: 0x1D00, last: 0x1D2B },
    UniRange { first: 0x1D6B, last: 0x1D77 },
    UniRange { first: 0x1D79, last: 0x1D7F },
    UniRange { first: 0x1D80, last: 0x1D9A },
    UniRange { first: 0x1DC0, last: 0x1DFF },
    UniRange { first: 0x1E00, last: 0x1EFF },  // Latin Extended Additional
    UniRange { first: 0x2000, last: 0x206F },  // General Punctuation
    UniRange { first: 0x2070, last: 0x209F },  // Superscripts/Subscripts
    UniRange { first: 0x20A0, last: 0x20CF },  // Currency Symbols
    UniRange { first: 0x2100, last: 0x214F },  // Letterlike Symbols
    UniRange { first: 0x2150, last: 0x218F },  // Number Forms
    UniRange { first: 0x2C60, last: 0x2C7F },  // Latin Extended-C
    UniRange { first: 0xA720, last: 0xA7FF },  // Latin Extended-D
    UniRange { first: 0xAB30, last: 0xAB6F },  // Latin Extended-E
    UniRange { first: 0xFB00, last: 0xFB06 },  // Alphabetic Presentation Forms
    UniRange { first: 0xFF00, last: 0xFFEF },  // Halfwidth/Fullwidth Forms
    UniRange { first: 0x1DF00, last: 0x1DFFF }, // Latin Extended-G
    UniRange { first: 0x10780, last: 0x107BF }, // Latin Extended-F
];
    ...
    for range in GRK_RANGES {
        ...
        while cp <= range.last {
            ...
            if cp > range.last || (cp - range.first > 512 && cmap.char_index(cp).is_none()) {
                ...
                cp = if skip > range.last { range.last + 1 } else { skip };
            }
        }
    }
