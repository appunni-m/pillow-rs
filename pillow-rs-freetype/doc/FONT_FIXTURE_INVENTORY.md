...
|---|---:|---:|---:|---:|---:|
| Active fixtures | 135 | 92 | 43 | 103 | 748 KiB |
...
| `script-coverage.ttf` | U+1E90C, U+0627, U+0531, U+0987, U+0A07, U+05D1, U+1781, U+0C87, U+2080, U+2070, U+0D12, U+1833, U+0E1A, U+A5CD, U+1900, U+0B13, U+A807, U+0F40 | 20 | explicit `FT_LOAD_FORCE_AUTOHINT` script-selection rows for Adlam, Arabic, Armenian, Bengali, Gurmukhi, Hebrew, Khmer, Kannada, Latin sub/superscript, Malayalam, Mongolian, Thai, Vai, and FreeType Indic CJK-writing-system paths for Limbu, Oriya, Syloti Nagri, and Tibetan |
...
| `hinter-control-matrix.ttf` | U+E032 gid 51 | 20 | branch-edge VM control covering zero-length line and stack vectors, invalid stack-index fallback, taken JROF, no-round dispatch, invalid contour shift, invalid coordinate reads, and empty twilight-zone SHZ |
...
30. One compact 5.5 KiB autohint script fixture provides one glyph per script
    ...
    `make font-fixture-autohint-scripts`. Current public inputs select 18 rows
    for script-standard-character reachability; the remaining glyphs are
    reserved for future explicit rows rather than an implicit script matrix.
