...
| `6355c637281e` | 2.0 | 1 | `fonts/type1/mm-blend-fontinfo-private.pfb` | source-backed compact Adobe Type 1 Multiple Master fixture with populated Private-dictionary blue, stem, and ForceBold fields; generated by `scripts/build_type1_fixtures.py` for the `t1tables.get_ps_font_private_mm_blend` batch |
| `a44b14975640` | 1.7 | 1 | `fonts/type1/non-mm-force-bold.pfb` | source-backed compact non-MM Type 1 ForceBold control generated by `scripts/build_type1_fixtures.py` for the `T1_BLEND_FORCE_BOLD` private-field comparison row |
| `e12821cbf928` | 1.9 | 1 | `fonts/type1-mm/adobe-mm-two-axis.pfb` | source-backed compact Adobe Type 1 Multiple Master descriptor fixture with Weight/Width axes, four designs, design maps, and default weight vector; generated by `scripts/build_type1_fixtures.py` for FTMM Adobe MM API parity |
| `e0211bea76fe` | 1.9 | 1 | `input/fonts/type1-mm/underline-position.pfb` | source-backed compact Adobe Type 1 Multiple Master fixture with a FontInfo `UnderlinePosition` blend array; generated by `scripts/build_type1_fixtures.py` for `t1tables.mm_blend_dictionary` public-output parity |
| `65e0aad861f6` | 1.9 | 1 | `input/fonts/type1-mm/underline-thickness.pfb` | source-backed compact Adobe Type 1 Multiple Master fixture with a FontInfo `UnderlineThickness` blend array; generated by `scripts/build_type1_fixtures.py` for `t1tables.mm_blend_dictionary` public-output parity |
...
| `ed5828a8809b`, `e19dd486f3a7`, `353ad0fc5ef8`, `7805c9993290`, `a4c5edb647e6`, `b5e2e7aa0565`, `6e8182784a25` | 2.0 each | 1 each | `fonts/cff/malformed-*.otf` | compact OpenType-CFF controls with malformed CFF short header, invalid INDEX offSize, truncated INDEX offset array, out-of-order INDEX offsets, escaped Top DICT op overflow, escaped non-CharStrings Top DICT op, and missing CharStrings operand; owns exact public `FT_New_Memory_Face` CFF face-open rejection branches |
| `57960be0c7e1`, `4a930bd85d28`, `6965600d0642`, `0196dc4247b3`, `9e32c0247aad` | 2.4 each | 1 each | `fonts/cff/malformed-top-dict-*.otf` | compact OpenType-CFF controls whose Top DICT contains longint, real, positive two-byte, negative two-byte, or invalid-number operand encodings; owns exact public `FT_New_Memory_Face` parity for pinned CFF Top DICT number parsing and Invalid_Table classification when parsed non-CharStrings operands still leave `CharStrings` absent |
...
| `8fa2f4748a2c` | 4.6 | 1 | `fixtures/assets/fonts/sbit_gray_index_format2_missing_big_metrics.ttf` | source-backed TrueType control with one 20 ppem EBLC/EBDT index-format-2 strike whose constant-size subtable stops inside the required big-metrics record, owning exact public `FT_LOAD_SBITS_ONLY` error parity for pinned `ttsbit.c` malformed constant-size metrics handling |
...
| `b2734479b1a1` | 4.9 | 1 | `fixtures/assets/fonts/sbit_gray_index_format4_sparse_miss.ttf` | source-backed TrueType control with one 20 ppem EBLC/EBDT strike whose range includes glyph 1 but whose index-format-4 sparse glyph/offset array contains only glyph 2 plus a sentinel, owning pinned `ttsbit.c` sparse scan miss and top-level NoBitmap public error parity |
...
| `c0b85b8dc8fd` | 4.9 | 1 | `fixtures/assets/fonts/sbit_mono_image_format5_missing_metrics.ttf` | source-backed TrueType control with one 20 ppem EBLC/EBDT index-format-1 subtable selecting bit-aligned image-format-5 data without EBLC-provided big metrics, owning the public malformed-SBIT error route for missing bit-aligned metrics |
| `6efa6ceef626` | 4.9 | 1 | `fixtures/assets/fonts/sbit_mono_index_format5_missing_big_metrics.ttf` | source-backed TrueType control with one 20 ppem EBLC/EBDT index-format-5 subtable whose constant metrics record is truncated before the sparse glyph-code count, owning exact public malformed constant-metrics error parity |
| `5fe7b538b31b` | 4.9 | 1 | `fixtures/assets/fonts/sbit_mono_index_format5_truncated_image.ttf` | source-backed TrueType control with one 20 ppem EBLC/EBDT index-format-5 bit-aligned bitmap whose EBDT image is shorter than the payload required by EBLC big metrics, owning pinned `ttsbit.c:858-864` malformed bit-aligned bitmap parity |
| `895fa7c5d432` | 4.9 | 1 | `fixtures/assets/fonts/sbit_mono_index_format5_sparse_miss.ttf` | source-backed TrueType control with one 20 ppem EBLC/EBDT strike whose range includes glyph 1 but whose index-format-5 sparse glyph-code array contains only glyph 2, owning pinned `ttsbit.c` constant-metrics sparse scan miss and top-level NoBitmap public error parity |
...
| `sbit_gray_index_format2_missing_big_metrics.ttf` | gid 1 | 20 | selected by `FT_Load_Glyph.matrix_load` with `FT_LOAD_SBITS_ONLY` to prove index-format-2 constant-size subtable truncation before big metrics returns the exact public C error |
...
| `sbit_gray_index_format4_sparse_miss.ttf` | gid 1 | 20 | selected by `FT_Load_Glyph.matrix_load` with `FT_LOAD_SBITS_ONLY` to prove EBLC index-format-4 range match followed by sparse glyph-array miss and exact public error parity |
...
| `sbit_mono_image_format5_missing_metrics.ttf`, `sbit_mono_index_format5_missing_big_metrics.ttf`, `sbit_mono_index_format5_truncated_image.ttf` | gid 1 | 20 | selected by `FT_Load_Glyph.matrix_load` with `FT_LOAD_SBITS_ONLY` to prove image-format-5 missing EBLC metrics, index-format-5 truncated big metrics, and bit-aligned EBDT payload truncation all return exact public C errors |
| `sbit_mono_index_format5_sparse_miss.ttf` | gid 1 | 20 | selected by `FT_Load_Glyph.matrix_load` with `FT_LOAD_SBITS_ONLY` to prove EBLC index-format-5 range match followed by sparse glyph-code miss and exact public error parity |
