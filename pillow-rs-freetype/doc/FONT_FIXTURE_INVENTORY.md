...
| `89225d625440` | 7.5 | 1 | `fonts/autohint/script-coverage.ttf` | source-backed glyf, 72 glyphs, 485 Unicode mappings, one compact glyph per autohint script tag plus standard-character aliases, Latin double-top, tilde branch, and serif-m probes |
...
| `b28e62f155a0` | 4.5 | 1 | `fixtures/assets/fonts/sbit_invalid_eblc_version.ttf` | source-backed TrueType control whose EBLC table has an unsupported version while EBDT is present; owns the invalid-SBIT-version public `FT_LOAD_SBITS_ONLY` error path |
...
| `script-coverage.ttf` | all generated `SCRIPT_PROBES` codepoints from `scripts/build_autohint_script_fixtures.py` plus Latin branch probes U+01D5, U+00F1, U+1E4D, U+00E3, U+00D1, U+1E1B, U+1E1A, U+1E75, and U+01D7 | 20 | explicit `FT_LOAD_FORCE_AUTOHINT` script-selection rows for compact script glyphs plus Latin double-top, top/top2/bottom tilde, and serif-m topology probes; this proves each selected script tag and the added Latin topology branches through real Rust/C ABI/WASM parity without an implicit script matrix |
...
30. One compact 7.5 KiB autohint script fixture provides one glyph per script
    ...
    generated script glyphs plus Latin double-top, top/top2/bottom tilde, and
    serif-m topology probes through explicit `FT_LOAD_FORCE_AUTOHINT` variants;
    this is a deliberate script/topology obligation set, not an implicit script
    matrix.
