...
| 2026-07-13 | Reject prep-range TrueType definitions like C | `hinter-prep-definitions.ttf` and `hinter-prep-idef.ttf` are derived compact TrueType controls whose prep programs attempt FDEF/IDEF definitions. Pinned C returns error 141 for these public loads, while Rust previously accepted the FDEF case; `ExecContext` now rejects definitions outside the font-program range and the two explicit `FT_Load_Glyph.matrix_load` rows prove exact Rust FFI, C ABI, and WASM ABI error parity. Concrete cases are 6,730 with zero implicit rows; runtime comparison is 6,726 / 6,726 with four explicit pending rows. Refreshed condition coverage is 15,873 / 17,765 lines, 22,782 / 25,454 regions, 3,803 / 4,526 branches, and 1,004 / 1,135 functions; missed source lines remain at 1,892 while this fixes a real C/Rust mismatch |

## Residual Coverage Classification - 2026-07-13

Fresh `test-unified-condition-coverage` still reports 1,892 uncovered core
