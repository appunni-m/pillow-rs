...
## 2026-07-19 WinFNT header fixtures and route

The `ftwinfnt.get_winfnt_header` and concrete `winfnt.get_header` charset rows
now use deterministic `.fnt` fixtures generated by
`scripts/generate_winfnt_fixtures.py`.

The pinned C behavior is:

- `FT_Get_WinFNT_Header` validates null face and null output pointer before
  calling the WinFNT service.
- Header retrieval is independent of active pixel size. The oracle and wrapper
  routes must open the face without forcing `FT_Set_Pixel_Sizes`; otherwise an
  8 px bitmap FNT can fail with `FT_Err_Invalid_Pixel_Size` before the public
  header endpoint is reached.
- FreeType copies the v3 `reserved1` payload as 16 raw bytes into
  `FT_ULong reserved1[4]`. On the maintained LP64 ABI this populates the first
  two 64-bit elements and leaves the remaining two elements zero.
...
The remaining WinFNT fixture gap is the distinct
