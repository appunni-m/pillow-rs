...
| `src/tt/hinter/iup.rs` | 2 | 102/104 (98.08%) | 0 | 2 | 6 |
...
| `tt/hinter/iup.rs:43-45,48,59,95` | Public-gate unreachable, C-oracle rejected, or defensive Rust slice guard | Pinned `TT_Load_Simple_Glyph` rejects duplicate/decreasing contour endpoints before bytecode, and valid zones keep point, tag, original, and unscaled arrays aligned. The former synthetic empty-contour path was removed: C's empty-glyph shortcut does not parse glyph instructions, while prep reaches `Ins_IUP` with `n_contours == 0` and returns before inspecting phantom points. Keep the remaining malformed-state bounds guards visible; do not add private or invalid-font rows to cover them |
