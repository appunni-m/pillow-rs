...
|---:|---|---|
| 269-270 | Caller-preempted | `prepare_context` skips `run_fpgm` when the font program is empty; the public empty-`fpgm` font already proves that outer route. |
| 446-447 | Caller invariant | `MD`, `MDRP`, `MIRP`, and `IP` use scaled `org` coordinates whenever any selected zone is twilight. `orus_in(zp=0)` is therefore not a legal opcode path. |
| 481-485 | Mixed invariant / malformed reference | Every live caller invokes `set_org_in` only after proving the selected zone is twilight, so its glyph-zone arm is redundant by caller contract. The failed bounds side requires an out-of-range twilight point and should be covered only with an exact normal/pedantic public error fixture. |
| 508-509 | Short-circuit invariant | SHPIX computes `in_twilight` first and never consults point tags on a twilight route; non-twilight routes always read glyph tags. |
| 546, 550, 760, 786 | Missing malformed-program route | NPUSH at end-of-code and unterminated IF/ELSE scans reach C `SkipCode` / `Ins_IF` / `Ins_ELSE` `Code_Overflow` behavior. Add compact malformed programs and fix Rust only if exact public error parity exposes a divergence. |
| 1513 | Public composite fixture gap | Requires v35/v40 compatibility state plus a composite glyph program using vertical freedom or a pre-Y-touched point in SHPIX. |
| 2053 | Public composite fixture gap | The remaining DELTAP side requires a composite glyph program with vertical freedom; the Y-touched simple-glyph path is already covered. |
| 2274 | Public font-program fixture gap | Requires valid `INSTCTRL` execution from the `fpgm` range. C permits selectors 1/2 only in prep and selector 3 only in a glyph program, making this a normal no-op / pedantic invalid-reference pair. |
