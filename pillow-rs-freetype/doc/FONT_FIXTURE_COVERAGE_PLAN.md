...
|---|---:|---|
| Real C/Rust/C-ABI/WASM parity route | 3,391 | Use these rows for structural coverage evidence. |
...
Fresh `test-unified-condition-coverage` still reports 1,815 uncovered core
...
| TrueType empty-zone SHZ | A derived `hinter-empty-composite-shz.ttf` font whose empty composite glyph carried `PUSHB[0] 0; SHZ[0]` | The focused `FT_Load_Glyph` row passed exact C/Rust/C-ABI/WASM parity, but full condition coverage stayed flat and `tt/hinter/exec.rs:1366` remained uncovered | Do not add this derived font. The empty-glyph-zone `SHZ` branch needs a different public construction route or a defensive-unreachable disposition |
