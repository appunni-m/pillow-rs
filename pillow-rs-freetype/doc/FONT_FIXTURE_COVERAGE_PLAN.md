...
| 2026-07-18 | SBIT index-format-2 constant-size lookup | `font-fixture-sbit` now emits `sbit_gray_index_format2.ttf`, a compact EBLC/EBDT control whose glyph 1 uses index format 2: constant image size plus EBLC big metrics, with a public image-format-1 8-bit gray bitmap payload. Pinned FreeType `sfnt/ttsbit.c:1302-1318` derives `image_start = image_size * (glyph_index - start)` and `image_end = image_start + image_size`; core now mirrors that lookup before routing to the existing byte-aligned image-format-1 decoder. Focused `FT_Load_Glyph.matrix_load@sbit-gray-index-format2-sbits-only` parity run `08be914d-87ac-4d29-a523-006fb3e31b13` passes, and focused condition coverage run `7ea047db-097b-4819-9099-1c58c6908d5b` ingests snapshot `b94ab5af-7b2a-4c87-b35e-ef86de9de310`. Full managed `fontdone-ftmm-real-parity-coverage` run `abc881c0-5405-4a81-a6b2-e7a410e01cf3` passes and ingests snapshot `2f827794-0f7b-4dd6-a4d8-4d5d690859ff`; route audit moves concrete cases 7,208 -> 7,209 and real-parity rows 3,654 -> 3,655 with route/core pending unchanged at 135. Against the previous full snapshot `4c297b97-3140-4346-9d89-8bd838a88305`, project covered lines move 20,573 -> 20,592 and covered regions 29,799 -> 29,831; `tt/sbit.rs` moves 514 / 638 -> 533 / 668 covered lines and 784 / 987 -> 816 / 1,034 covered regions while branch coverage remains 60 / 60. |
| 2026-07-18 | SBIT index-format-4 sparse lookup | `font-fixture-sbit` now emits `sbit_gray_index_format4.ttf`, a compact EBLC/EBDT control whose glyph 1 uses index format 4 sparse `(glyph, offset)` pairs plus the required sentinel pair, with a public image-format-1 8-bit gray bitmap payload. Pinned FreeType `sfnt/ttsbit.c:1332-1365` scans sparse glyph codes and derives the selected image end from the following pair's offset; core now mirrors that lookup before routing to the existing byte-aligned image-format-1 decoder. Focused `FT_Load_Glyph.matrix_load@sbit-gray-index-format4-sbits-only` parity run `2b38e64d-214c-49d9-9e16-ad9802b0079a` passes, and focused condition coverage run `8ab1c845-b55f-42da-a126-ac4cbbfefdf9` ingests snapshot `98e8a43a-9afb-4a72-a722-db32000f3506`. Full managed `fontdone-ftmm-real-parity-coverage` run `7450e062-67f4-43d5-be9a-c28aed2aebd3` passes and ingests snapshot `c977cd42-0f02-4e41-a885-be8e2a593229`; route audit moves concrete cases 7,209 -> 7,210 and real-parity rows 3,655 -> 3,656 with route/core pending unchanged at 135. Against the previous full snapshot `2f827794-0f7b-4dd6-a4d8-4d5d690859ff`, project covered lines move 20,592 -> 20,622 and covered regions 29,831 -> 29,892; `tt/sbit.rs` moves 533 / 668 -> 563 / 725 covered lines and 816 / 1,034 -> 877 / 1,132 covered regions, while branch coverage moves 60 / 60 -> 61 / 62. |
| 2026-07-18 | SBIT index-format-5 bit-aligned lookup | `font-fixture-sbit` now emits `sbit_mono_index_format5.ttf`, a compact EBLC/EBDT control whose glyph 1 uses index format 5 constant image size, EBLC big metrics, and a sparse glyph-code array, with image format 5 bit-aligned 1-bit bitmap payload. Pinned FreeType `sfnt/ttsbit.c:1367-1401` scans the sparse glyph-code array and computes `image_start = image_size * matched_index`; `sfnt/ttsbit.c:1180-1182` selects the bit-aligned image-format-5 loader, and `sfnt/ttsbit.c:828-955` copies the continuous MSB-first bitstream into the row-aligned target bitmap. Core now carries EBLC metrics on the selected image record and mirrors the bit-aligned MONO decode for the public `FT_LOAD_SBITS_ONLY` path. Focused `FT_Load_Glyph.matrix_load@sbit-mono-index-format5-sbits-only` parity run `6d0b1e88-bef2-4581-98f3-67b8b9669385` passes, and focused condition coverage run `ed9849d5-5758-43ed-85d5-1535fcaa1322` ingests snapshot `4de111e7-7c2a-44db-8a38-a812ea26c8e4`. Full managed `fontdone-ftmm-real-parity-coverage` run `61365e58-1aa0-4c25-83b3-d26ca0d2f886` passes and ingests snapshot `de02c99e-590b-4f06-8174-e974df57dc01`; route audit moves concrete cases 7,210 -> 7,211 and real-parity rows 3,656 -> 3,657 with route/core pending unchanged at 135. Against the previous full snapshot `c977cd42-0f02-4e41-a885-be8e2a593229`, project covered lines move 20,622 -> 20,708 and covered regions 29,892 -> 30,043; `tt/sbit.rs` moves 563 / 725 -> 649 / 871 covered lines and 877 / 1,132 -> 1,028 / 1,354 covered regions, while branch coverage moves 61 / 62 -> 68 / 70. |
| 2026-07-18 | SBIT sparse-miss public errors | `font-fixture-sbit` now emits `sbit_gray_index_format4_sparse_miss.ttf` and `sbit_mono_index_format5_sparse_miss.ttf`, compact EBLC/EBDT controls whose selected 20 ppem strike ranges include glyph 1 but whose sparse arrays contain only glyph 2. Pinned FreeType `sfnt/ttsbit.c:1332-1365` and `sfnt/ttsbit.c:1367-1401` scan the sparse entries, fail to find glyph 1, and take the top-level NoBitmap path at `sfnt/ttsbit.c:1430-1441`; `truetype/ttgload.c:2401-2469` maps the failed scalable `FT_LOAD_SBITS_ONLY` load to public `FT_Err_Invalid_Argument`. Rust already mirrored that scan and no-bitmap behavior; these rows prove it through exact Rust FFI, C ABI, and WASM ABI public error comparison rather than leaving the C-observable miss path uncovered. Focused sparse-miss parity run `92068555-8dd6-4edc-8d25-e16774572f9c` passes, and focused condition coverage run `bc6e942c-d749-4e4c-ae7f-275ecef992d5` ingests snapshot `e08b0516-8d0a-4820-8cb6-fd4a7d08d1d9`, proving `tt/sbit.rs` lines 331, 354, 387, and 413 execute. Full managed `fontdone-ftmm-real-parity-coverage` run `533144f0-1596-46e2-8388-a9b0c6aef925` passes and ingests snapshot `f041795b-fb6c-4ea9-a506-e088fae2afbb`; route audit moves concrete cases 7,211 -> 7,213 and real-parity rows 3,657 -> 3,659 with route/core pending unchanged at 135. Against the previous full snapshot `de02c99e-590b-4f06-8174-e974df57dc01`, project covered lines move 20,708 -> 20,712, covered branches 4,971 -> 4,973, and covered regions 30,043 -> 30,047; `tt/sbit.rs` moves 649 / 871 -> 653 / 871 covered lines, 68 / 70 -> 70 / 70 covered branches, and 1,028 / 1,354 -> 1,032 / 1,354 covered regions. |
| 2026-07-18 | CFF malformed face-open exact errors | `font-fixture-cff` now emits three additional malformed OpenType-CFF controls for a truncated INDEX offset array, a complete escaped non-CharStrings Top DICT operator followed by missing CharStrings, and a CharStrings operator with no operand. The grouped `FT_New_Memory_Face.error_malformed_cff_table` row now compares exact public error output for all seven malformed CFF variants. Pinned FreeType 2.14.3 classifies CFF table framing/header/INDEX failures as `FT_Err_Invalid_Table` in `cffload.c`, Top DICT escaped-operator/operand failures as public `FT_Err_Invalid_Argument` through `cffparse.c`/`cffload.c`, and the out-of-order INDEX case as `FT_Err_Invalid_File_Format`. Rust previously collapsed six of these paths through `FontError::InvalidFont` to error code 3, so enabling exact error comparison exposed the mismatch. Core now has a dedicated `FontError::InvalidTable`, maps it through Rust FFI, C ABI, and WASM/test conversion as `FT_Err_Invalid_Table`, and keeps the Top DICT underflow paths as `InvalidArgument`. Focused parity run `43fd7157-28f7-4882-b7e1-bdf89f18418f` passes, focused condition coverage run `5c6864a3-e9b7-4703-862d-7810494d2fc0` ingests snapshot `0bd1d6e0-436a-42a7-809f-f03f4201e27c`, and full managed `fontdone-ftmm-real-parity-coverage` run `5f83969a-8348-4a3b-9e60-819a21e5b3ee` passes with snapshot `a0fe2a18-6f58-45d6-a2a5-071c79564e0a`. Runtime parity is 7,081 / 7,081 with 135 explicit pending rows. Route audit keeps concrete cases at 7,216 while moving the seven CFF rows from generic-error fallback to exact real parity: real-parity rises 3,659 -> 3,666 and generic-error-fallback drops 466 -> 459. Against previous full snapshot `0056a4fe-3968-4e5c-a993-9e8da7d647ed`, project covered lines move 20,723 / 21,746 -> 20,724 / 21,747 and covered regions move 30,061 / 31,814 -> 30,062 / 31,815 because the new `InvalidTable` conversion path is covered; `tt/cff.rs` remains 453 / 509 lines, 82 / 82 branches, and 645 / 762 regions because the structural CFF parser lines were already executed before exact-error comparison was enabled. |
| 2026-07-18 | CFF Top DICT operand parsing | `font-fixture-cff` now emits five compact OpenType-CFF controls whose Top DICT contains longint, real, positive two-byte, negative two-byte, or invalid-number operand encodings. The four valid operand encodings are consumed by the numeric escaped `UnderlinePosition` operator (`cfftoken.h` operator `12 3`) and then leave the required `CharStrings` offset absent; byte `31` exercises the invalid DICT number class directly. Pinned FreeType 2.14.3 parses DICT integer and real operands in `cffparse.c:cff_parse_integer` / `cff_parse_real`, then reports public `FT_Err_Invalid_Table` from `cffload.c` for a Top DICT with parsed non-CharStrings operands but no `CharStrings`; empty/minimal Top DICT and CharStrings operand-underflow shapes still preserve `FT_Err_Invalid_Argument`. Rust previously returned `InvalidArgument` for all missing-CharStrings shapes and `InvalidFont` for invalid number bytes. Core now tracks whether Top DICT parsing consumed non-CharStrings operands and maps those C-observable missing-CharStrings failures, plus invalid DICT number bytes, to `InvalidTable` while retaining the older underflow classes. Focused parity run `fdc8f39b-4482-42ef-8628-231aed2cecfd` passes, and focused condition coverage run `78a35e44-00f5-4f3e-9d51-4bfefca992aa` ingests snapshot `3481d24d-78e3-4328-8c75-4d1ac0b5e752`. Full managed `fontdone-ftmm-real-parity-coverage` run `90538268-4841-45e3-b6da-df9b70d11e4d` passes and ingests snapshot `72cce5dc-677e-4730-b71f-828665b2bb2e`; runtime parity is 7,090 / 7,090 with 135 explicit pending rows, route audit reports 7,225 concrete cases and real-parity 3,675, and no runtime-native-FFI/fmt/clippy gates fail. Against full baseline snapshot `d75aac4b-ebab-4411-b053-863a9b7e22f4`, project covered lines move 20,734 / 21,747 -> 20,765 / 21,752, covered branches 4,973 / 5,565 -> 4,977 / 5,569, covered functions 1,290 / 1,451 -> 1,291 / 1,451, and covered regions 30,076 / 31,815 -> 30,128 / 31,820. `tt/cff.rs` moves 453 / 509 -> 484 / 514 lines, 82 / 82 -> 86 / 86 branches, 33 / 52 -> 34 / 52 functions, and 645 / 762 -> 697 / 767 regions, newly covering baseline-missed lines 183, 184, 186, 187, 196, 200-204, 207, 211, 214, 220, 223, 225, 226, 228, 232, and 617. |
...
| 2026-07-16 | TrueType VM execution-too-long route | `hinter-execution-too-long-loop.ttf` is generated by `build_hinter_edge_fixtures.py` from the compact hinter control matrix with gid 1 replaced by a negative `JMPR` loop. The original `FT_Load_Glyph.matrix_load` row exercised the pure-Rust VM guard but used permissive expected-error acceptance, so it did not prove the pinned C error or its non-pedantic suppression behavior. The pure-Rust VM guard returns `FT_Err_Execution_Too_Long` instead of generic invalid outline. Focused parity passed the permissive 1 / 1 row. Full condition coverage passed with 7,036 / 7,036 runtime rows and the same three FTMM pending rows; `src/tt/hinter/exec.rs` moved from 1,351 / 1,379 lines, 2,741 / 2,946 regions, and 381 / 416 branches to 1,352 / 1,379 lines, 2,742 / 2,946 regions, and 382 / 416 branches. Route audit real-parity rose from 3,792 to 3,793. The later exact pedantic/non-pedantic section records the corrected oracle proof. |
...
| 2026-07-17 | Latin linked neutral-blue topology | `arabic-standard-fallback.ttf` now maps U+0640 TATWEEL to a flat neutral-zone sample at y=300 and U+0628 to a compact stem linking that edge to the normal Arabic bottom blue at y=0. One explicit `FT_Load_Glyph.matrix_load@arabic-linked-neutral-blue-stem-force-autohint` row follows pinned `afblue.dat` and `aflatin.c:2529-2640,4276-4290`; exact C oracle, Rust FFI, C ABI, and WASM ABI parity passes 1 / 1. Full Coverage MCP parity rises from 7,031 / 7,031 to 7,032 / 7,032 with 153 unchanged pending rows; concrete cases rise 7,184 to 7,185 and route-audit real parity rises 3,607 to 3,608. `src/autohint/latin.rs` moves from 2,621 / 2,838 lines, 1,084 / 1,284 branches, and 3,821 / 4,212 regions to 2,624 / 2,838 lines, 1,086 / 1,284 branches, and 3,824 / 4,212 regions; functions remain 67 / 68. Normalized exact changes complete the neutral assignment predicate at line 1814 and linked-neutral predicate at line 4151, with line 4152 newly covered. |
...
## Exact TrueType execution-limit behavior

The original `hinter-execution-too-long-loop` row was false-green: it declared
`expect_error` without comparing the error payload, while pinned FreeType
actually returned success because `FT_LOAD_PEDANTIC` was absent.  Adding the
pedantic flag first exposed a second fixture problem: the control-matrix
font's intentionally invalid shared `fpgm`/`prep` programs made C fail earlier
with `FT_Err_Invalid_Reference`, while Rust reached the glyph loop and returned
`FT_Err_Execution_Too_Long`.

The maintained generator now clears those shared programs before installing
the negative `JMPR` glyph loop.  Two distinct public rows then pin both sides
of FreeType's behavior: without `FT_LOAD_PEDANTIC`, `TT_Hint_Glyph` suppresses
the interpreter error and loading succeeds (`ttgload.c:828-837`); with
`FT_LOAD_PEDANTIC`, the negative-jump/interpreter limit returns
`FT_Err_Execution_Too_Long` (`ttinterp.c:3201-3218,6730-6741`).  Both rows
compare pinned C, Rust FFI, C ABI, and WASM ABI output exactly.

Focused Coverage MCP run `fd5d9d99-ad49-4835-bdf0-ed263883a812` passes both
rows.  Full managed run `d08f9ac6-44c7-4617-9186-7f0ccb334e07` passes all
7,070 runnable rows with zero failures, 135 pending rows, 7,205 concrete rows,
zero implicit rows, and 3,651 real-parity routes, and ingests snapshot
`52854a95-1a36-46fd-9a84-f226a12ad2c4`.  Against
`d4ba47d7-052c-4ead-9dbb-5ade86957072`, coverage moves from 20,561 / 21,513
to 20,562 / 21,513 lines and from 29,771 / 31,447 to 29,772 / 31,447 regions;
branches remain 4,958 / 5,555 and functions remain 1,281 / 1,417.  The sole
newly covered source line is `src/ffi/convert.rs:202`, proving the public
`ExecutionTooLong` conversion instead of merely reaching the core guard.

The wait-time audit classified `tt/sbit.rs`'s remaining executable gaps as
platform-impossible overflow/error closures with all 60 branch outcomes
already covered, and `tt/cff.rs`'s gaps as malformed-input closures or ignored
DICT-number forms with all 82 branch outcomes covered.  It also confirmed that
`AF_ADJUST_TILDE_BOTTOM2` remains unreachable because pinned FreeType defines
the bit without assigning it to a character.  The next real public candidate
is the SDF walker's valid cubic-close-to-start arm: append a CFF glyph whose
explicit endpoint equals its start, let the pinned CFF builder behavior remove
