...
fallback behavior. The former `tt/fvar.rs` and `tt/gasp.rs` checked arithmetic
closures were resolved on 2026-07-16 by matching C's bounded header validation
and direct count arithmetic; the maintained malformed public rows cover every
...
| Covered body with closure artifact | `Font::load_sfnt_table` closures, `Font::truetype_face_with_load_mode` closures, `SizeMetrics::from_char_size` closure, `tt::cmap` checked-overflow closures, `api::GlyphSlot::new` closure | Do not add fixture rows for closure symbols alone. Add rows only if a public error branch or output difference is missing |
