...
maintained generated fixture, including Type 1 MM design-coordinate partial,
extra, and reset scenarios plus the no-default-named-instance Type 1 MM
service path; broader OpenType variation coordinate mutation, glyph
interpolation output, and `FT_MM_Var` allocation
...
| `FT_Get_MM_Var` | `freetype/ftmm.h` | `FT_Error FT_Get_MM_Var(FT_Face face, FT_MM_Var **amaster)` | Partially implemented for generated Type 1 MM descriptors and maintained OpenType `fvar` descriptors: `FT_MM_Var` counts, `FT_Var_Axis` records, axis flags, namedstyle coordinate records, and `FT_Done_MM_Var` release status are compared through Rust FFI, C ABI, and WASM ABI. Hidden-axis-specific and CFF2 descriptor rows remain planned. |
...
| `FT_Set_Var_Design_Coordinates` | `freetype/ftmm.h` | `FT_Error FT_Set_Var_Design_Coordinates(FT_Face face, FT_UInt num_coords, FT_Fixed *coords)` | Partially implemented for maintained OpenType `fvar` fixtures: default, after-set, partial/excess/reset state, public design-coordinate preservation, normalized internal-coordinate conversion, metrics variation, and variation flag behavior are compared through Rust FFI, C ABI, and WASM ABI. Glyph-output variation remains planned. |
| `FT_Get_Var_Design_Coordinates` | `freetype/ftmm.h` | `FT_Error FT_Get_Var_Design_Coordinates(FT_Face face, FT_UInt num_coords, FT_Fixed *coords)` | Partially implemented for maintained OpenType `fvar` fixtures and named-instance state. The declared excess-output zero-fill row remains pending because pinned FreeType 2.14.3 produced non-zero run-varying excess-slot data for the current single-axis fixture. |
| `FT_Set_MM_Blend_Coordinates` | `freetype/ftmm.h` | `FT_Error FT_Set_MM_Blend_Coordinates(FT_Face face, FT_UInt num_coords, FT_Fixed *coords)` | Partially implemented for generated Type 1 MM coordinate state: normalized set, partial/excess behavior, reset, getter observation, and variation flag behavior are compared through Rust FFI, C ABI, and WASM ABI. OpenType calls preserve C's public-wrapper sentinel behavior; glyph-output variation remains pending until a maintained Adobe MM fixture has interpolating glyph programs. |
| `FT_Get_MM_Blend_Coordinates` | `freetype/ftmm.h` | `FT_Error FT_Get_MM_Blend_Coordinates(FT_Face face, FT_UInt num_coords, FT_Fixed *coords)` | Partially implemented for maintained OpenType variation and generated Type 1 MM state rows, including default, after-set, partial/excess count, and null/error behavior through Rust FFI, C ABI, and WASM ABI. |
| `FT_Set_Var_Blend_Coordinates` | `freetype/ftmm.h` | `FT_Error FT_Set_Var_Blend_Coordinates(FT_Face face, FT_UInt num_coords, FT_Fixed *coords)` | Partially implemented for maintained OpenType variation state: alias distinction from `FT_Set_MM_Blend_Coordinates`, variation flag transitions, partial/excess/reset behavior, and glyph-output routes are compared through Rust FFI, C ABI, and WASM ABI. |
| `FT_Get_Var_Blend_Coordinates` | `freetype/ftmm.h` | `FT_Error FT_Get_Var_Blend_Coordinates(FT_Face face, FT_UInt num_coords, FT_Fixed *coords)` | Partially implemented for maintained OpenType variation and generated Type 1 MM state rows, including deterministic excess-output zero-fill for the single-axis blend fixture through Rust FFI, C ABI, and WASM ABI. |
...
service-without-default-callback row. OpenType `FT_MM_Var` now covers the
maintained TrueType `fvar` descriptor row, including axis flags and namedstyle
coordinates. Hidden-axis-specific rows, CFF2 descriptors, variation state APIs,
and glyph-output interpolation remain planned.
...
- Complete `fvar` hidden-axis fixtures, CFF2 descriptors, synthesized default
  instances, and default coordinate edge cases.
...
cmap format-14 UVS queries.  Compact generated fixtures now exist at
`fonts/type1-mm/adobe-mm-two-axis.pfb` and the legacy manifest path
`fonts/mm/adobe-multiple-master.pfb`; pinned C FreeType opens both as
two-axis/four-design MM faces.  Returning successful ABI stubs before
pure-Rust Type 1 MM parsing, state mutation, allocation ownership, and
oracle-backed dynamic tests exist would create false compatibility.
Descriptor, weight-vector, design-coordinate, default-named-instance, and
reset rows are real only where the same generated fixture passes through
pinned C FreeType, Rust FFI, thin C ABI, and WASM ABI with exact output;
remaining `FT_MM_Var`, broader named-instance, and glyph-output rows must stay
pending until they have the same proof.  Glyph-output rows additionally
require real Type 1 MM interpolation.
