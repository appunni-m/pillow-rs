...
| `fterrdef.FT_Err_Missing_Property.known_property_success` | Focused run: `runnable=0 pending=1`. The fixture declares `module_name="svg"` / `property_name="svg-hooks"`, while pinned FreeType exposes that property on module `ot-svg`. | Do not remap `svg` to `ot-svg` inside the route. That would compare a different public input and hide the fixture-contract issue. |
