...
fn main() {
    ...
    if out.is_empty() || out == "NO_GLYPH" {
        eprintln!("FT: NO_GLYPH");
        return;
    }
    let parts: Vec<&str> = out.split_whitespace().collect();
    if parts.len() < 5 {
        eprintln!("FT parse fail");
        return;
    }
    let (w, h, l, t) = (
    ...
    let ft_px: Vec<u8> = (0..parts[4].len())
        .step_by(2)
        .map(|i| u8::from_str_radix(&parts[4][i..i + 2], 16).unwrap())
        .collect();
    ...
    if our_sha == ft_sha {
        ...
        for i in 0..min {
            if mask.pixels[i] != ft_px[i] {
                eprintln!(
                    "  [{}] our=0x{:02x} ft=0x{:02x}",
                    i, mask.pixels[i], ft_px[i]
                );
            }
        }
