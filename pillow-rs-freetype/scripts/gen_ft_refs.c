...
static FixtureFamily family_from_name(const char* name) {
    ...
    if (streq(name, "render-lcd-v") || streq(name, "render_lcd_v")) {
        return (FixtureFamily){"render_lcd_v",
                               FT_LOAD_FORCE_AUTOHINT | FT_LOAD_TARGET_LCD_V,
                               "[\"FT_LOAD_FORCE_AUTOHINT\",\"FT_LOAD_TARGET_LCD_V\"]",
...
static int emit_batch(const char* font_path, FixtureFamily family) {
    FT_Library library;
    FT_Face face;
    unsigned char* buf = NULL;
    long sz = 0;
    ...
    for (int si = 0; si < 5; si++) {
        ...
        for (int c = 33; c < 127; c++) {
            FT_UInt idx = FT_Get_Char_Index(face, (FT_ULong)c);
            if (!idx) {
                continue;
            }
            err = FT_Load_Glyph(face, idx, family.load_flags);
            if (!err && family.render_after_load) {
                err = FT_Render_Glyph(face->glyph, family.render_mode);
            }
            if (err) {
            ...
            if (family.render_after_load) {
                printf(" PITCH %d PIXEL_MODE %u", pitch, slot->bitmap.pixel_mode);
            }
            if (w > 0 && h > 0 && slot->bitmap.buffer) {
                printf(" PIXELS");
                int bytes = family.render_after_load ? labs(pitch) * h : w * h;
                for (int i = 0; i < bytes; i++) {
                    printf(" %02x", slot->bitmap.buffer[i]);
                }
            }
            printf("\n");
        }
...
int main(int argc, char** argv) {
    ...
    if (argc - arg == 2) {
        return emit_batch(argv[arg], family_from_name(argv[arg + 1]));
    }
    if (argc - arg < 3) {
        fprintf(stderr,
                "Usage: %s [--json] <font.ttf> <CP_HEX> <size_pt> [fixture_family]\n"
                "       %s <font.ttf> <fixture_family>\n",
                argv[0],
                argv[0]);
        return 1;
    }
