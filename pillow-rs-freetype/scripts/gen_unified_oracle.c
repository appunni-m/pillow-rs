...
static int emit_stroker_conic_success(int argc, char** argv) {
    (void)argv;
    if (argc != 2) return 2;
    FT_Library library = NULL;
    FT_Error init_error = FT_Init_FreeType(&library);
    ...
    FT_Vector to = { 512, 0 };
    FT_Error begin_error = FT_Stroker_BeginSubPath(stroker, &start, 0);
    FT_Error conic_error = begin_error ? begin_error : FT_Stroker_ConicTo(stroker, &control, &to);
    FT_Error end_error = conic_error ? conic_error : FT_Stroker_EndSubPath(stroker);
    FT_UInt point_count = 0;
    FT_UInt contour_count = 0;
    FT_Error counts_error = end_error ? end_error : FT_Stroker_GetCounts(stroker, &point_count, &contour_count);
    FT_Vector points[128] = {0};
    ...
    printf("\"status_sequence\":[%d,%d,%d],", begin_error, conic_error, end_error);
    printf("\"point_count\":%u,\"contour_count\":%u,", point_count, contour_count);
    printf("\"exported_outline\":");
    print_stroker_outline_json(&exported);
...
static int emit_stroker_conic_first_segment(int argc, char** argv) {
    (void)argv;
    if (argc != 2) return 2;
    FT_Library library = NULL;
    FT_Error init_error = FT_Init_FreeType(&library);
...
static int dispatch(int argc, char** argv) {
    ...
    if (argc == 2 && streq(argv[1], "--stroker-conic-success")) {
        return emit_stroker_conic_success(argc, argv);
    }
