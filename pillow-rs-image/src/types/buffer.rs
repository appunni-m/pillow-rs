...
mod test {
    ...
    fn test_get_put_pixel() {
        let mut img: RgbImage = ImageBuffer::new(3, 3);
    ...
    fn test_from_raw() {
        let data = vec![0u8; 9];
        let img: ImageBuffer<Luma<u8>, _> = ImageBuffer::from_raw(3, 3, data).unwrap();
        assert_eq!(img.dimensions(), (3, 3));
    }

    #[test]
    fn test_from_fn() {
        let img = RgbImage::from_fn(2, 2, |x, y| Rgb([x as u8, y as u8, 0]));
        assert_eq!(*img.get_pixel(0, 0), Rgb([0, 0, 0]));
        assert_eq!(*img.get_pixel(1, 0), Rgb([1, 0, 0]));
        assert_eq!(*img.get_pixel(0, 1), Rgb([0, 1, 0]));
        assert_eq!(*img.get_pixel(1, 1), Rgb([1, 1, 0]));
    }

    #[test]
    fn test_from_pixel() {
        let img = RgbImage::from_pixel(2, 2, Rgb([42, 43, 44]));
        for y in 0..2 {
            for x in 0..2 {
                assert_eq!(*img.get_pixel(x, y), Rgb([42, 43, 44]));
            }
        }
    }
    ...
    fn test_index() {
        ...
        img[(0, 0)] = Rgb([1, 2, 3]);
        assert_eq!(img[(0, 0)], Rgb([1, 2, 3]));
    }

    #[test]
    fn test_deref() {
        let img: ImageBuffer<Luma<u8>, Vec<u8>> =
            ImageBuffer::from_raw(2, 2, vec![1u8, 2, 3, 4]).unwrap();
        assert_eq!(img.len(), 4);
        assert_eq!((*img)[0], 1u8);
    }
    ...
    #[test]
    fn test_default() {
        let img: RgbImage = ImageBuffer::default();
