...
pub fn pil_grayscale(img: &DynamicImage) -> image::GrayImage {
    ...
    // Precompute LUTs matching PIL's exact rounding
    let mut lut_r = [0u16; 256];
    let mut lut_g = [0u16; 256];
    let mut lut_b = [0u16; 256];
    for i in 0..256 {
        lut_r[i] = ((0.299 * i as f64 + 0.5) as u16).min(255);
        lut_g[i] = ((0.587 * i as f64 + 0.5) as u16).min(255);
        lut_b[i] = ((0.114 * i as f64 + 0.5) as u16).min(255);
    }

    let mut gray = vec![0u8; n];
    let mut i = 0;
    let len = rgb_data.len().min(n * 3);
    // Single tight loop — no per-pixel function calls
    while i + 2 < len {
        let r = rgb_data[i] as usize;
        let g = rgb_data[i + 1] as usize;
        let b = rgb_data[i + 2] as usize;
        gray[i / 3] = (lut_r[r] + lut_g[g] + lut_b[b]).min(255) as u8;
        i += 3;
    }
